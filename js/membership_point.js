...
$(document).ready(function (){
    ...
    buildSelectorContent(API_ROOT_URL + "/t/sy/dictionary/findBoOrder",
        {"groupKey":"INTEGRAL_SERVICE_TYPE"},
        true,
        "itemKey",
        "itemValue",
        ["#businessTypeScore"],null);
...
function buildCondData() {//查找数据条件
    var cond = {};
    // 资源类型
    var billType = $("#billTypeScore").children('option:selected').val();
    if (billType == null)
        billType = "";
    // 层级 
    var businessType = $("#businessTypeScore").children('option:selected').val();
...
function loadResourceData(conditionData) {
    // ajax加载资源表格数据
    ajaxRequest(API_ROOT_URL + routeUrl +"getUserScoreList", conditionData, false, function (result) {
        var arr = result.data;
...
function buildTableRow(rowData) {
    var resource_row = "<tr id=" + '"' + rowData["acScoreLogId"] + '"' + ">";
    // 状态列
    ...
    // 资源名称列
    resource_row += buildTableCol(rowData["businessName"]);
    // 资源key列
