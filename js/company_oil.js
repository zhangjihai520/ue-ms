...
function loadUserData(conditionData) {
    // ajax加载账户表格数据
    ajaxRequest(API_ROOT_URL + routeUrl + "findCompanyAll",
        conditionData, false,
        function (result) {
            var arr = result.data;
        ...
        // } );
            ...
            $('#buyer-account-dialog').on('show.bs.modal', function (event) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    var dict = arr[i];
                    if (dict["cpCompanyId"] == curSelectId) {
                        buyerAccount = dict["accountList"];
                        break;
                    }
                }
                var src = '';
            ...
            $('#company-audit-dialog').on('show.bs.modal', function (event) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    if (dict["cpCompanyId"] == curSelectId) {
                        name = dict["name"];
                        uscc = dict["uscc"];
                        detailedAddress = dict["addressAll"]+dict["address"];
...
function buildTableRow(rowData) {
    var user_row = "<tr id=" + '"' + rowData["cpCompanyId"] + '"' + ">";
    // 账户名列
    ...
    user_row += buildTableCol(rowData["addressAll"]+rowData["address"]);
    // 创建时间列
    user_row += buildTableCol(getDate(rowData["createTime"]));
    // 状态列
    var statusFlag = rowData["authStatus"];
    var statusSpan = "";
    if (statusFlag == 0) {
    ...
    var opt = '<div class="d-lg-flex flex-lg-row"> \
              <button disabled="disabled" type="button" name="company-account" id="company-account" \
                class="btn btn-info btn-sm ml-1 mr-1 mt-1 mb-1 company-account" data-toggle="modal" data-target="#buyer-account-dialog">关联账户</button> \
              <button disabled="disabled" type="button" name="company-oil-audit" id="company-oil-audit" \
                class="btn btn-danger btn-sm ml-1 mr-1 mt-1 mb-1 company-oil-audit" data-toggle="modal" data-target="#company-audit-dialog">公司审核</button> \
              </div>';
