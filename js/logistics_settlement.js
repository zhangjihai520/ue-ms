...
$(document).ready(function () {
    ...
    // 获取物流商列表框选项数据
    buildSelectorContent(API_ROOT_URL + "/t/tr/carrier/findCarrierAll",
        null,
        true,
        "trCarrierId",
        "name",
...
function buildCondData() {
    ...
    cond["object_type"] = "FS0_LOGISTICS_PROVIDER";
    cond["cycle"] = cycle;
    cond["status"] = status;
...
function buildTableRow(rowData) {
    ...
    if (statusFlag == 0) {
        ...
        if (bo) {
            ...
            class="btn btn-info btn-sm ml-1 mr-1 mt-1 mb-1 fiCheckToBeConfirmedCarrier" data-toggle="modal" \
            data-target="#delete-user-dialog">确认账单</button>';
        }
        ...
        if (systemBo) {
            ...
            class="btn btn-success btn-sm ml-1 mr-1 mt-1 mb-1 fiCheckCarrierPrepaid" data-toggle="modal" \
    ...
    opt += '<button disabled="disabled" type="button" name="checkTheDetails" id="checkTheDetails" \
                class="btn btn-danger btn-sm ml-1 mr-1 mt-1 mb-1 checkTheCarrierDetails" data-toggle="modal" \
                data-target="#check-the-details-dialog">查看明细</button> \
              <button disabled="disabled" type="button" name="downloadFiCheckDetails" id="downloadFiCheckDetails" \
                class="btn btn-primary btn-sm ml-1 mr-1 mt-1 mb-1 downloadCarrierFiCheckDetails">下载明细</button> \
              </div>';
