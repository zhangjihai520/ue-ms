...
function loadUserData(conditionData) {
    // ajax加载账户表格数据
    ajaxRequest(API_ROOT_URL + routeUrl + "findCsBuyerAll",
        conditionData, false,
        function (result) {
            var arr = result.data;
        ...
        // } );
            ...
            $('#buyer-account-dialog').on('show.bs.modal', function (event) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    var dict = arr[i];
                    if (dict["csServerId"] == curSelectId) {
...
function buildTableRow(rowData) {
    var user_row = "<tr id=" + '"' + rowData["csServerId"] + '"' + ">";
    // 账户名列
    ...
    var authLevel = rowData["level"];
    var authLevelName = authLevel + '级会员';
    ...
    var opt = '<div class="d-lg-flex flex-lg-row"> \
              <button disabled="disabled" type="button" name="buyer-account" id="buyer-account" \
                class="btn btn-info btn-sm ml-1 mr-1 mt-1 mb-1 buyer-account-cs" data-toggle="modal" data-target="#buyer-account-dialog">关联账户</button> \
              <button disabled="disabled" type="button" name="company-audit" id="company-audit" \
                class="btn btn-danger btn-sm ml-1 mr-1 mt-1 mb-1 company-audit-cs" data-toggle="modal" data-target="#company-audit-dialog">公司审核</button> \
              </div>';
